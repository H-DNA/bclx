...
inline void rwrite_sync(const T *src, const gptr<T> &dst, const size_t &size) {
  MPI_Put(src, size * sizeof(T), MPI_CHAR, dst.rank, dst.ptr, size * sizeof(T),
          MPI_CHAR, BCL::win);
  MPI_Win_flush(dst.rank, BCL::win);
}

template <typename T, typename U>
inline void rwrite_sync(const std::vector<T> &src, const gptr<U> &dst,
                        const U &type) {
  MPI_Put(src.data(), type.get_elem_count() * sizeof(T), MPI_CHAR, dst.rank,
          dst.ptr, 1, type.get_elem_type(), BCL::win);
  MPI_Win_flush(dst.rank, BCL::win);
}
...
template <typename T, typename U>
inline void rwrite_async(const std::vector<T> &src, const gptr<U> &dst,
                         const U &type) {
  MPI_Put(src.data(), src.size() * sizeof(T), MPI_CHAR, dst.rank, dst.ptr, 1,
          type.get(), BCL::win);
}
...
template <typename T>
inline void rwrite_block2(const T *src, const gptr<T> &dst,
                          const size_t &size) {
  MPI_Request request;
  MPI_Status status;
...
inline void awrite_sync(const T *src, const gptr<T> &dst, const size_t &size) {
  MPI_Accumulate(src, size * sizeof(T), MPI_CHAR, dst.rank, dst.ptr,
                 size * sizeof(T), MPI_CHAR, MPI_REPLACE, BCL::win);
  MPI_Win_flush(dst.rank, BCL::win);
}
...
inline void rread_sync(const gptr<T> &src, T *dst, const size_t &size) {
  MPI_Get(dst, size * sizeof(T), MPI_CHAR, src.rank, src.ptr, size * sizeof(T),
          MPI_CHAR, BCL::win);
...
inline void rread_sync(const gptr<U> &src, std::vector<T> &dst, const U &type) {
  MPI_Get(dst.data(), type.get_elem_count() * sizeof(T), MPI_CHAR, src.rank,
          src.ptr, 1, type.get_elem_type(), BCL::win);
  MPI_Win_flush(src.rank, BCL::win);
}
...
inline void aread_sync(const gptr<T> &src, T *dst, const size_t &size) {
  ...
  MPI_Get_accumulate(origin_addr, 0, MPI_CHAR, dst, size * sizeof(T), MPI_CHAR,
                     src.rank, src.ptr, size * sizeof(T), MPI_CHAR, MPI_NO_OP,
                     BCL::win);
  MPI_Win_flush(src.rank, BCL::win);
}
...
template <typename T, typename U>
inline void fetch_and_op_sync(const gptr<T> &dst, const T *val,
                              const BCL::atomic_op<U> &op, T *result) {
  MPI_Fetch_and_op(val, result, op.type(), dst.rank, dst.ptr, op.op(),
                   BCL::win);
  MPI_Win_flush(dst.rank, BCL::win);
}

template <typename T>
inline void compare_and_swap_sync(const gptr<T> &dst, const T *old_val,
                                  const T *new_val, T *result) {
  ...
  MPI_Compare_and_swap(new_val, old_val, result, datatype, dst.rank, dst.ptr,
                       BCL::win);
  MPI_Win_flush(dst.rank, BCL::win);
}
...
inline void barrier_async() { MPI_Barrier(BCL::comm); }

template <typename T>
inline void scatter(const T *src_buf, T *dst_buf, const size_t &src_rank,
                    const size_t &size) {
  MPI_Scatter(src_buf, size * sizeof(T), MPI_CHAR, dst_buf, size * sizeof(T),
              MPI_CHAR, src_rank, BCL::comm);
}

template <typename T>
inline void alltoall(const T *src_buf, T *dst_buf, const size_t &size) {
  MPI_Alltoall(src_buf, size * sizeof(T), MPI_CHAR, dst_buf, size * sizeof(T),
               MPI_CHAR, BCL::comm);
}

template <typename T, typename U>
inline void reduce(const T *src_buf, T *dst_buf, const size_t &dst_rank,
                   const BCL::atomic_op<U> &op, const size_t &size) {
  MPI_Reduce(src_buf, dst_buf, size, op.type(), op.op(), dst_rank, BCL::comm);
}

template <typename T, typename U>
inline void allreduce(const T *src_buf, T *dst_buf, const BCL::atomic_op<U> &op,
                      const size_t &size) {
...
inline void recv(T *dst_buf, const size_t &src_rank, const size_t &size) {
  MPI_Status status;
  MPI_Recv(dst_buf, size * sizeof(T), MPI_CHAR, src_rank, 0, BCL::comm,
           &status);
}
