...
#define pointerto(item, object_ptr)                                            \
  BCL::reinterpret_pointer_cast<GET_MEMBER_TYPE(                               \
      &std::remove_reference<decltype(object_ptr)>::type::type::item)>(        \
      BCL::reinterpret_pointer_cast<char>(object_ptr) +                        \
      offsetof(                                                                \
          typename std::remove_reference<decltype(object_ptr)>::type::type,    \
          item))

template <typename T> struct GlobalPtr;
...
template <typename T> struct GlobalPtr {
  ...
  GlobalPtr(GlobalPtr &&other)
      : rank(std::move(other.rank)), ptr(std::move(other.ptr)) {
    other = nullptr;
  }
  ...
  bool operator!=(const std::nullptr_t null) const { return !(*this == null); }
  ...
  bool is_local() const { return rank == BCL::rank(); }
  ...
  // custom SHMEM.
  T *rptr() const { return (T *)(((char *)BCL::smem_base_ptr) + ptr); }

  GlobalRef<T> operator*() { return GlobalRef<T>(*this); }

  const GlobalRef<T> operator*() const { return GlobalRef<T>(*this); }

  GlobalRef<T> operator[](size_type idx) { return *(*this + idx); }

  const GlobalRef<T> operator[](size_type idx) const { return *(*this + idx); }
  ...
  template <typename U> class CallHandler {
  public:
    ...
    ~CallHandler() { *ptr_ = *reinterpret_cast<U *>(obj_); }

    BCL::GlobalPtr<U> ptr_;
    char obj_[sizeof(U)];
  };

  CallHandler<T> operator->() { return CallHandler<T>(*this); }

  const CallHandler<T> operator->() const { return CallHandler<T>(*this); }

  std::string str() const {
    if (*this != nullptr) {
      return "GlobalPtr(" + std::to_string(rank) + ": " + std::to_string(ptr) +
             ")";
    } else {
