...
public:
	...
	std::vector<dds::pool_ubd_mpsc>			pipes;		// SPSC unbounded pools
		...
		: size_class{sc}, scan_count{0}
{
	...
	for (uint64_t i = 0; i < BCL::nprocs(); ++i)
	{
		...
		pipes.push_back(dds::pool_ubd_mpsc(i, obj_size, true, true, nullptr));
	}
}
...
{
	// initialize @pipes
	pipes.push_back(dds::pool_ubd_mpsc(BCL::rank(), size + 16, false, false, nullptr));
}
