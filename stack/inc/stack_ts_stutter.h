...
template<typename T>
dds::tss_stutter::stack<T>::stack(const uint64_t &num)
{
	//synchronize
	BCL::barrier();
...
template <typename T>
bool dds::tss_stutter::stack<T>::push(const T &value)
{
        timestamp		ts;
...
template <typename T>
bool dds::tss_stutter::stack<T>::push_fill(const T &value)
{
	gptr<elem<T>>		oldTopAddr,
				newTopAddr;
	elem<T>			newTopVal;
