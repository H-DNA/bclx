#ifndef MEMORY_TRIAL_H
#define MEMORY_TRIAL_H
...
template<typename T>
bclx::gptr<T> dds::trial::memory<T>::malloc()
{
...
template<typename T>
void dds::trial::memory<T>::free(const gptr<T>& ptr)
{
...
template<typename T>
void dds::trial::memory<T>::retire(const gptr<T>& ptr)
{
...
template<typename T>
void dds::trial::memory<T>::op_begin() {}

template<typename T>
void dds::trial::memory<T>::op_end() {}

template<typename T>
bclx::gptr<T> dds::trial::memory<T>::try_reserve(const gptr<gptr<T>>& ptr, const gptr<T>& val_old)
{
	if (val_old == nullptr)
		return nullptr;
...
template<typename T>
bclx::gptr<T> dds::trial::memory<T>::reserve(const gptr<gptr<T>>& ptr)
{
...
template<typename T>
void dds::trial::memory<T>::unreserve(const gptr<T>& ptr)
{
	if (ptr == nullptr)
		return;
	else // if (ptr != nullptr)
...
template<typename T>
void dds::trial::memory<T>::empty2()
{
	for (uint64_t i = 0; i < queues[BCL::rank()].size(); ++i)
	{
		std::vector<gptr<T>> slist;
